...
public class SleepTypeActivity extends AppCompatActivity {
    ...
    // [검색/정렬] 리스트 분리
    private List<SleepType> originalList = new ArrayList<>();
    private List<SleepType> filteredList = new ArrayList<>();
    ...
    private void filterList(String text) {
        ...
        if (text.isEmpty()) {
            ...
            for (SleepType item : originalList) {
    ...
    private void addType() {
        ...
        // [중복 체크]
        for (SleepType type : originalList) {
        ...
        apiService.insertSleepType(newType).enqueue(new Callback<Void>() {
            ...
            public void onResponse(Call<Void> call, Response<Void> response) {
                if (response.isSuccessful()) {
                    etName.setText("");
                    fetchTypes();
                    Toast.makeText(SleepTypeActivity.this, "추가됨", Toast.LENGTH_SHORT).show();
                }
            }
            @Override
    ...
    private void returnSelectedType(String name) {
        ...
        intent.putExtra("selected_sleep_type", name);
